  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-[0.32em] text-sky-300">{role} dashboard</p>
          <h3 className="mt-3 text-2xl font-semibold text-white">{headline}</h3>
        </div>
        <div className="rounded-3xl bg-slate-900 px-4 py-2 text-sm text-slate-200">Live preview</div>
      </div>
      <p className="mt-5 text-sm leading-7 text-slate-400">{summary}</p>
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        {stats.map((stat) => (
          <div key={stat.label} className="rounded-3xl border border-slate-800/80 bg-slate-900 p-4">
